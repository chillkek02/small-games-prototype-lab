...
const CATEGORY_SOURCES = {
  driving: '/en/driving', puzzle: '/en/puzzle', action: '/en/action', simulation: '/en/simulation',
  sports: '/en/sports', dressup: '/en/dress-up', skill: '/en/skill', adventure: '/en/adventure'
};

const ENGINE_OPTIONS = [
  { id:'auto', label:'Factory Recommended', description:'Let the Factory choose for both gameplay and premium presentation.' },
...
const ART_OPTIONS = [
  ...
  { id:'toy3d', label:'Gutpopper Bright Toy 3D' },
  { id:'isometric', label:'Polished Isometric 2.5D' },
  { id:'vector', label:'Bright Layered Vector Casual' },
  { id:'voxel', label:'Bright Blocky / Voxel 3D' },
  ...
  { id:'minimal', label:'Minimal Clean — intentional only' }
];
...
const CONCEPT_LIBRARY = [
  ...
  },
  {
    ...
    engine:'three', art:'toy3d', mobileFit:98, adFit:90, buildFit:92,
    why:['3D before/after cleanup is immediately satisfying','Simple touch controls','Strong level and tool-upgrade structure','Toy-town props and particles give it premium screenshot appeal']
  },
  {
  ...
  },
  {
    ...
    engine:'phaser3', art:'isometric', mobileFit:91, adFit:82, buildFit:90,
    why:['Puzzle clarity works well in portrait or landscape','Layered isometric art prevents a flat board-game feel','Short levels support rapid iteration','Easy to expand with modifiers']
  },
  {
    ...
    engine:'three', art:'toy3d', mobileFit:93, adFit:89, buildFit:84,
    why:['Strong readable stakes','3D fire/smoke/rescue staging creates visual spectacle','Short missions fit browser sessions','Natural second-chance rewarded ad']
  },
  {
    ...
    engine:'three', art:'voxel', mobileFit:92, adFit:84, buildFit:82,
    why:['One-tap setup and payoff loop','3D collapse moments are highly GIF-able','Levels can reuse modular geometry','Scores and bonus objectives drive replay']
  },
  {
    ...
    engine:'three', art:'toy3d', mobileFit:97, adFit:93, buildFit:87,
    why:['Very clear progression fantasy','Toy-shop characters/props create strong visual personality','Queue pressure adds action','Rewarded boosts fit naturally']
  },
  {
...
const FALLBACK_SIGNAL = {
  driving:{demand:84,saturation:72,popularOverlap:5,newOverlap:8}, puzzle:{demand:82,saturation:83,popularOverlap:4,newOverlap:12},
  action:{demand:80,saturation:86,popularOverlap:4,newOverlap:14}, simulation:{demand:78,saturation:68,popularOverlap:3,newOverlap:14},
  sports:{demand:74,saturation:70,popularOverlap:2,newOverlap:7}, dressup:{demand:73,saturation:75,popularOverlap:1,newOverlap:10},
  skill:{demand:79,saturation:80,popularOverlap:3,newOverlap:11}, adventure:{demand:70,saturation:72,popularOverlap:1,newOverlap:6}
};
...
async function fetchText(url, timeoutMs=7000) {
  ...
  try {
    const response=await fetch(url,{signal:controller.signal,headers:{'user-agent':'Mozilla/5.0 GutpopperGameFactory/0.12 market-research','accept-language':'en-US,en;q=0.8'}});
...
function extractGameSlugs(html='') {
  const set=new Set();
  const patterns=[/href=["']\/en\/g\/([^"'?#/]+)[^"']*["']/gi,/href=["']https:\/\/poki\.com\/en\/g\/([^"'?#/]+)[^"']*["']/gi];
  for(const pattern of patterns){let match;while((match=pattern.exec(html)))set.add(match[1].toLowerCase())}
  return set;
}
...
async function collectSignals() {
  ...
  try {
    ...
    const popular=extractGameSlugs(popularHtml), fresh=extractGameSlugs(newHtml), stats={};
    Object.keys(CATEGORY_SOURCES).forEach((category,index)=>{
      ...
      stats[category]={demand:clamp(52+popularOverlap*7+Math.min(newOverlap,8)*1.5),saturation:clamp(35+Math.min(total,120)*.48+Math.min(newOverlap,14)*1.5),popularOverlap,newOverlap,observedGames:total};
    });
    return {live:true,checkedAt:new Date().toISOString(),sources,stats};
  } catch(error) {
    return {live:false,checkedAt:new Date().toISOString(),sources,error:error.message,stats:FALLBACK_SIGNAL};
  }
}

function scoreConcept(concept,signal) {
  const demand=signal?.demand??70, saturation=signal?.saturation??75, whitespace=100-saturation;
  const score=clamp(demand*.34+whitespace*.19+concept.mobileFit*.20+concept.adFit*.14+concept.buildFit*.13);
  return {...concept,score,demand,saturation,whitespace:clamp(whitespace),confidence:score>=82?'High':score>=72?'Medium-High':'Medium'};
}

export async function getOpportunityReport() {
  const market=await collectSignals();
  const opportunities=CONCEPT_LIBRARY.map(concept=>scoreConcept(concept,market.stats[concept.category])).sort((a,b)=>b.score-a.score);
  return {platform:'Poki',generatedAt:new Date().toISOString(),market,opportunities,methodology:'Scores combine observed Poki category/popular/new-page overlap with mobile fit, monetization fit, production fit, visual-production fit, and category saturation. This ranks opportunities; it does not guarantee commercial performance.',engines:ENGINE_OPTIONS,artStyles:ART_OPTIONS};
}
...
export function resolveRecommendation({concept='',engine='auto',artStyle='auto',category=''}={}) {
  ...
  if(!candidate){
    if(/drive|truck|car|tow|delivery|vehicle|road|parking|taxi|bus/.test(normalized)) candidate={engine:'three',art:'toy3d'};
    else if(/clean|vacuum|wash|repair|restore|job|rescue|fire|crowd|shop|tycoon|store|restaurant|worker|construction|demol|destroy|city|town|world|3d/.test(normalized)) candidate={engine:'three',art:'toy3d'};
    else if(/puzzle|sort|belt|conveyor|grid|match|card|word/.test(normalized)) candidate={engine:'phaser3',art:'isometric'};
    else if(/platform|shooter|sports|runner|arcade|survival|arena/.test(normalized)) candidate={engine:'phaser3',art:'vector'};
    else candidate={engine:'three',art:'toy3d'};
  }
  return {engine:engine==='auto'?candidate.engine:engine,artStyle:artStyle==='auto'?candidate.art:artStyle};
}
