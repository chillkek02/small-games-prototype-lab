export const VISUAL_QUALITY_FLOOR = {
  version: '1.1.0',
  minimumPrototypeScore: 78,
  publishCandidateScore: 85,
  hardFailCap: 59,
  categories: [
    { id: 'artDirection', label: 'Art direction / cohesion', weight: 15 },
    { id: 'depthLighting', label: 'Depth / lighting / material treatment', weight: 15 },
    { id: 'worldRichness', label: 'World / asset richness', weight: 15 },
    { id: 'gameFeel', label: 'Game feel / feedback', weight: 15 },
    { id: 'animationPolish', label: 'Animation / motion polish', weight: 10 },
    { id: 'uiTypography', label: 'UI / typography', weight: 10 },
    { id: 'composition', label: 'Composition / responsive layout', weight: 10 },
    { id: 'readability', label: 'Readability / silhouettes', weight: 5 },
    { id: 'finish', label: 'Professional finish / personality', weight: 5 }
  ],
  hardFails: [
    { id: 'placeholder_primitive_dominance', label: 'Major characters, vehicles, props, environment, or UI still read primarily as raw placeholder rectangles/circles/primitives.' },
    ...
    { id: 'flat_depthless_scene', label: 'Gameplay scene is visually flat: weak/no lighting, grounding shadows, layered depth, material treatment, atmospheric separation, or perspective cues.' },
    { id: 'lifeless_animation', label: 'Core gameplay presentation is mostly static/linear with little easing, secondary motion, reaction, squash/pop, particles, trails, camera response, or celebration/failure motion.' },
    ...
    { id: 'incoherent_style', label: 'The screen mixes incompatible colors, fonts, panel styles, shape languages, materials, or asset treatments without a deliberate visual system.' }
  ]
};
...
export function visualQualityBrief() {
  ...
  return `GUTPOPPER VISUAL QUALITY FLOOR v${VISUAL_QUALITY_FLOOR.version}\nA first playable prototype is not acceptable merely because it works. It must already look like a professionally directed modern casual game prototype.\n\nMINIMUM FIRST-PROTOTYPE BAR\n- Weighted visual score must be at least ${VISUAL_QUALITY_FLOOR.minimumPrototypeScore}/100.\n- No visual hard-fail condition may be present.\n- Aim for ${VISUAL_QUALITY_FLOOR.publishCandidateScore}/100+ when practical without bloating scope or load time.\n\nSCORING\n${categoryText}\n\nHARD FAILS\n${failText}\n\nGENERATION RULES\n- Do not use raw primitive boxes/circles as the final visual language for major characters, environments, vehicles, props, or HUD unless they are intentionally stylized into cohesive multi-part assets with layering, detail, hierarchy, shadows/lighting, materials, and personality.\n- Brightness alone is not polish. Use controlled saturation, warm/cool contrast, highlights, shadows and readable focal hierarchy.\n- A scene needs depth: contact/soft shadows, overlap, perspective/isometric separation, foreground/midground/background, lighting gradients, fog/atmosphere or equivalent 2D layering.\n- Surfaces should have deliberate material identity rather than one flat default look. Use roughness/specular contrast, highlights, gradients, rim/outline treatment, emissive accents, procedural shader effects, texture, or equivalent styling where appropriate to the art direction.\n- Lightweight procedural shaders are encouraged when they improve quality-per-byte, but shader usage is not itself a quality requirement. Do not add expensive fullscreen postprocessing merely to satisfy the visual rubric.\n- Avoid giant flat sidebars, beige/debug panels, raw bordered boxes, default-looking controls, and spreadsheet/dashboard composition.\n- Establish a deliberate palette, material/shape language, typography hierarchy, UI language, scene density, lighting strategy and feedback language before filling the screen.\n- Desktop and phone may use different composition rules. Desktop should feel intentionally landscape; phone should feel intentionally touch-first.\n- Include enough environment/prop variety and layered detail that the scene does not read as an empty test room.\n- Core actions need professional motion and juice: anticipation/easing, squash/pop, secondary motion, particles/trails, flashes, floating feedback, camera response, success/failure beats.\n- Vehicles/characters/props should read as designed multi-part objects rather than single-color primitives.\n- Simple mechanics are allowed. Crude, flat, default, empty, lifeless, or programmer-art presentation is not.\n- Protect fast loading and mobile performance while meeting the visual bar.`;
}
