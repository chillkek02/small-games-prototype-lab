...
export const QUALITY_JOB_VERSION='1.0.0';
...
export async function handleQualityJobsApi({req,res,url,stateDir,gameInfo,sendJson,gameIsBusy,qualityBusy,host,port}){
  const latest=url.pathname.match(/^\/api\/games\/([^/]+)\/doctor\/latest$/);
  if(req.method==='GET'&&latest){const game=decodeURIComponent(latest[1]);if(!safe(game))return sendJson(res,400,{error:'Invalid game id'});const runs=(await listRuns(stateDir)).filter(x=>x.game===game);return sendJson(res,200,{run:runs[0]||null})}
  ...
  if(req.method==='POST'&&start){
    const game=decodeURIComponent(start[1]);if(!safe(game))return sendJson(res,400,{error:'Invalid game id'});if(gameIsBusy(game))return sendJson(res,409,{error:'Wait for the active Factory operation to finish before running Game Doctor.'});const info=await gameInfo(game);if(!info)return sendJson(res,404,{error:'Game target not found'});
    const id=`doctor-${Date.now()}-${randomUUID().slice(0,8)}`,run={version:QUALITY_JOB_VERSION,id,game,status:'running',percent:1,stage:'Starting Game Doctor',detail:'Preparing the quality pipeline…',createdAt:new Date().toISOString(),startedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),result:null,error:null};await writeRun(stateDir,run);qualityBusy.add(game);
    const worker=new Promise(resolve=>setImmediate(resolve)).then(async()=>{
      try{const result=await runQualityAudit({game,gameDir:info.gameDir,url:`http://${host}:${port}/game/${encodeURIComponent(game)}/`,stateDir,onProgress:async update=>{await patchRun(stateDir,id,{percent:update.percent,stage:update.stage,detail:update.detail})}});await patchRun(stateDir,id,{status:'completed',percent:100,stage:'Game Doctor complete',detail:`Overall ${result.overallScore}/100 · visual ${result.visualScore}/100`,finishedAt:new Date().toISOString(),result})}catch(error){await patchRun(stateDir,id,{status:'failed',stage:'Game Doctor failed',detail:error.message,error:error.message,finishedAt:new Date().toISOString()})}finally{qualityBusy.delete(game);activeRuns.delete(id)}});
