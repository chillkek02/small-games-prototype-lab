...
async function readBody(req){let raw='';for await(const chunk of req){raw+=chunk;if(raw.length>250000)throw new Error('Request body too large')}return raw?JSON.parse(raw):{}}

async function listGames(){
  const entries=await fsp.readdir(GAMES_DIR,{withFileTypes:true});const games=[];
  for(const entry of entries){if(!entry.isDirectory())continue;try{const html=await fsp.readFile(path.join(GAMES_DIR,entry.name,'index.html'),'utf8');const title=html.match(/<title[^>]*>([^<]+)<\/title>/i)?.[1]?.trim()||entry.name;let metadata=null;try{metadata=JSON.parse(await fsp.readFile(path.join(GAMES_DIR,entry.name,'factory-game.json'),'utf8'))}catch{}games.push({id:entry.name,title,url:`/game/${encodeURIComponent(entry.name)}/`,metadata})}catch{}}
  return games.sort((a,b)=>a.id.localeCompare(b.id,undefined,{numeric:true}));
}

async function serveFile(res,filePath,{noCache=false}={}){
  try{const stat=await fsp.stat(filePath);if(!stat.isFile())return false;res.writeHead(200,{'content-type':MIME[path.extname(filePath).toLowerCase()]||'application/octet-stream','content-length':stat.size,'cache-control':noCache?'no-store':'public, max-age=60','x-content-type-options':'nosniff'});fs.createReadStream(filePath).pipe(res);return true}catch{return false}
}
...
async function snapshotBeforeBuild({game,gameDir,label,kind,jobId=null}){return createSnapshot({stateDir:STATE_DIR,game,gameDir,label,kind,jobId})}

async function handleApi(req,res,url){
  if(req.method==='GET'&&url.pathname==='/api/status'){
    const[codex,jobs]=await Promise.all([probeCodex(REPO_ROOT),store.list(5)]);
    return sendJson(res,200,{name:'Gutpopper Game Factory',version:'0.13.0',repoRoot:REPO_ROOT,codex,engines:{phaser3:'3.90.0',phaser4:'4.2.1',three:'0.185.1'},qualityLab:{visualDirector:true,studioVisualDirector:true,houseStyle:'Gutpopper Bright Toy Casual v1',assetForge:true,visualMinimum:78,visualCommercialTarget:85,aiPlaytester:true,retentionReplay:true,adReadiness:true,performanceGate:true,pokiReadiness:true,desktopViewport:'1440x900',phoneViewport:'390x844'},studioLoop:{version:STUDIO_LOOP_VERSION,defaultNewGames:true,defaultExistingEdits:false,...DEFAULT_STUDIO_LOOP},snapshots:{automatic:true,maxPerGame:12,undo:true},gameTrash:{safeDelete:true,recoverable:true},activeGames:[...activeByGame.keys()],recentJobs:jobs.length});
  }
  ...
  if(req.method==='DELETE'&&deleteGameMatch){const game=decodeURIComponent(deleteGameMatch[1]);if(!safeSegment(game))return sendJson(res,400,{error:'Invalid game id'});if(gameIsBusy(game))return sendJson(res,409,{error:'Wait for the active Factory operation to finish before deleting this game.'});const gameDir=gameDirFor(game);if(!await validateGameDir(gameDir))return sendJson(res,404,{error:'Game target not found'});let title=game;try{const html=await fsp.readFile(path.join(gameDir,'index.html'),'utf8');title=html.match(/<title[^>]*>([^<]+)<\/title>/i)?.[1]?.trim()||game}catch{}try{const trashed=await trashGame({stateDir:STATE_DIR,game,gameDir,title});return sendJson(res,200,{deleted:true,game,title,trash:{trashId:trashed.trashId,deletedAt:trashed.deletedAt,recoverable:true}})}catch(error){return sendJson(res,500,{error:`Could not move game to Factory Trash: ${error.message}`})}}

  const snapshotListMatch=url.pathname.match(/^\/api\/games\/([^/]+)\/snapshots$/);
  if(snapshotListMatch){const game=decodeURIComponent(snapshotListMatch[1]);if(!safeSegment(game))return sendJson(res,400,{error:'Invalid game id'});const gameDir=gameDirFor(game);if(!await validateGameDir(gameDir))return sendJson(res,404,{error:'Game target not found'});if(req.method==='GET')return sendJson(res,200,{snapshots:await listSnapshots({stateDir:STATE_DIR,game})});if(req.method==='POST'){if(gameIsBusy(game))return sendJson(res,409,{error:'Wait for the active Factory operation to finish before saving a restore point.'});try{const body=await readBody(req);return sendJson(res,201,{snapshot:await createSnapshot({stateDir:STATE_DIR,game,gameDir,label:String(body.label||'Manual restore point').slice(0,120),kind:'manual'})})}catch(error){return sendJson(res,500,{error:`Could not create restore point: ${error.message}`})}}}

  const undoMatch=url.pathname.match(/^\/api\/games\/([^/]+)\/undo$/);
  if(req.method==='POST'&&undoMatch){const game=decodeURIComponent(undoMatch[1]);if(!safeSegment(game))return sendJson(res,400,{error:'Invalid game id'});if(gameIsBusy(game))return sendJson(res,409,{error:'Wait for the active Factory operation to finish before restoring.'});const gameDir=gameDirFor(game);if(!await validateGameDir(gameDir))return sendJson(res,404,{error:'Game target not found'});restoreBusy.add(game);try{const target=(await listSnapshots({stateDir:STATE_DIR,game}))[0];if(!target)return sendJson(res,404,{error:'No restore point exists for this game yet.'});const safety=await createSnapshot({stateDir:STATE_DIR,game,gameDir,label:`Before restoring ${target.label||target.id}`,kind:'pre-restore'}),restored=await restoreSnapshot({stateDir:STATE_DIR,game,gameDir,snapshotId:target.id});return sendJson(res,200,{restored,safetySnapshot:safety,snapshots:await listSnapshots({stateDir:STATE_DIR,game})})}catch(error){return sendJson(res,500,{error:`Restore failed: ${error.message}`})}finally{restoreBusy.delete(game)}}

  const restoreMatch=url.pathname.match(/^\/api\/games\/([^/]+)\/restore\/([^/]+)$/);
  if(req.method==='POST'&&restoreMatch){const game=decodeURIComponent(restoreMatch[1]),snapshotId=decodeURIComponent(restoreMatch[2]);if(!safeSegment(game)||!safeSegment(snapshotId))return sendJson(res,400,{error:'Invalid restore request'});if(gameIsBusy(game))return sendJson(res,409,{error:'Wait for the active Factory operation to finish before restoring.'});const gameDir=gameDirFor(game);if(!await validateGameDir(gameDir))return sendJson(res,404,{error:'Game target not found'});restoreBusy.add(game);try{const safety=await createSnapshot({stateDir:STATE_DIR,game,gameDir,label:'Before manual restore',kind:'pre-restore'}),restored=await restoreSnapshot({stateDir:STATE_DIR,game,gameDir,snapshotId});return sendJson(res,200,{restored,safetySnapshot:safety})}catch(error){return sendJson(res,500,{error:`Restore failed: ${error.message}`})}finally{restoreBusy.delete(game)}}

  const doctorMatch=url.pathname.match(/^\/api\/games\/([^/]+)\/doctor$/);
  if(req.method==='POST'&&doctorMatch){const game=decodeURIComponent(doctorMatch[1]);if(!safeSegment(game))return sendJson(res,400,{error:'Invalid game id'});if(gameIsBusy(game))return sendJson(res,409,{error:'Wait for the active Factory operation to finish before running Game Doctor.'});const gameDir=gameDirFor(game);if(!await validateGameDir(gameDir))return sendJson(res,404,{error:'Game target not found'});qualityBusy.add(game);try{return sendJson(res,200,await runQualityAudit({game,gameDir,url:`http://${HOST}:${PORT}/game/${encodeURIComponent(game)}/`,stateDir:STATE_DIR}))}catch(error){return sendJson(res,500,{error:`Game Doctor failed: ${error.message}`})}finally{qualityBusy.delete(game)}}
  ...
  if(req.method==='POST'&&url.pathname==='/api/new-games'){
    ...
    try{const project=await createGameProject({gamesDir:GAMES_DIR,factoryDir:__dirname,title:body.title,concept:body.concept,engine:body.engine||'auto',artStyle:body.artStyle||'auto',opportunity:body.opportunity||'',target:body.target||'Poki'});const created=await store.create({game:project.id,instruction:project.instruction});const baseline=await snapshotBeforeBuild({game:project.id,gameDir:project.gameDir,label:'Initial Production + House Style + Asset Forge before first AI build',kind:'new-game-baseline',jobId:created.id});const studioLoop=normalizeStudioLoop(body.studioLoop||{},{defaultEnabled:true});const job=await store.patch(created.id,{status:'running',stage:'New game build',attempt:1,kind:'new-game',creator:project.metadata,studioLoop,snapshotId:baseline.id,error:null});await store.appendLog(job.id,`New Game Creator scaffolded ${project.id} · ${project.engine} · ${project.artStyle} · ${project.houseStyle?.name||'house style'} · Asset Forge ${project.assetKit?.version||'ready'}`);await store.appendLog(job.id,`Studio Loop ${studioLoop.enabled?'ON':'OFF'} · up to ${studioLoop.maxIterations} deep rounds · visual target ${studioLoop.visualTarget}`);await store.appendLog(job.id,`Safety snapshot saved · ${baseline.id}`);dispatchWorker({job,game:project.id,gameDir:project.gameDir});return sendJson(res,202,{game:{id:project.id,title:project.title,url:`/game/${encodeURIComponent(project.id)}/`,metadata:project.metadata},job:await store.get(job.id)})}catch(error){return sendJson(res,400,{error:error.message})}finally{creatingGame=false}
  }

  if(req.method==='POST'&&url.pathname==='/api/jobs'){
    let body;try{body=await readBody(req)}catch(error){return sendJson(res,400,{error:error.message})};const game=String(body.game||''),instruction=String(body.instruction||'').trim();if(!safeSegment(game))return sendJson(res,400,{error:'Invalid game id'});if(instruction.length<4)return sendJson(res,400,{error:'Describe the change you want the Factory to make.'});if(gameIsBusy(game))return sendJson(res,409,{error:`${game} already has an active Factory operation.`});const gameDir=gameDirFor(game);if(!await validateGameDir(gameDir))return sendJson(res,404,{error:'Game target not found'});const created=await store.create({game,instruction});let safety;try{safety=await snapshotBeforeBuild({game,gameDir,label:`Before: ${instruction.replace(/\s+/g,' ').slice(0,90)}`,kind:'pre-build',jobId:created.id})}catch(error){await store.patch(created.id,{status:'failed',stage:'Snapshot failed',finishedAt:new Date().toISOString(),error:error.message});return sendJson(res,500,{error:`Factory refused to edit without a safety snapshot: ${error.message}`})}const studioLoop=normalizeStudioLoop(body.studioLoop||{},{defaultEnabled:false});const job=await store.patch(created.id,{status:'running',stage:'Dispatching Factory',attempt:1,studioLoop,snapshotId:safety.id,error:null});await store.appendLog(job.id,`Safety snapshot saved · ${safety.id}`);await store.appendLog(job.id,`Studio Loop ${studioLoop.enabled?'ON':'OFF'}${studioLoop.enabled?` · up to ${studioLoop.maxIterations} deep rounds`:''}`);dispatchWorker({job,game,gameDir});return sendJson(res,202,await store.get(job.id));
  }
  return false;
}

async function handler(req,res){
  const url=new URL(req.url,`http://${req.headers.host||`${HOST}:${PORT}`}`);if(url.pathname.startsWith('/api/')){const handled=await handleApi(req,res,url);if(handled!==false)return}
  const qualityArtifactMatch=url.pathname.match(/^\/quality-artifacts\/([^/]+)\/([^/]+)$/);if(qualityArtifactMatch&&safeSegment(qualityArtifactMatch[1])&&safeSegment(qualityArtifactMatch[2])){const file=safeJoin(path.join(STATE_DIR,'quality',qualityArtifactMatch[1]),qualityArtifactMatch[2]);if(file&&await serveFile(res,file,{noCache:true}))return;res.writeHead(404);res.end('Not found');return}
  const artifactMatch=url.pathname.match(/^\/artifacts\/([^/]+)\/([^/]+)$/);if(artifactMatch&&safeSegment(artifactMatch[1])&&safeSegment(artifactMatch[2])){const file=safeJoin(store.jobDir(artifactMatch[1]),artifactMatch[2]);if(file&&await serveFile(res,file,{noCache:true}))return;res.writeHead(404);res.end('Not found');return}
