...
async function readBody(req){let raw='';for await(const chunk of req){raw+=chunk;if(raw.length>300000)throw new Error('Request body too large')}return raw?JSON.parse(raw):{}}
...
async function handleApi(req,res,url){
  if(req.method==='GET'&&url.pathname==='/api/status'){
    ...
    return sendJson(res,200,{name:'Gutpopper Game Factory',version:'0.15.0',repoRoot:REPO_ROOT,codex,engines:{phaser3:'3.90.0',phaser4:'4.2.1',three:'0.185.1'},qualityLab:{visualDirector:true,studioVisualDirector:true,houseStyle:'Gutpopper Bright Toy Casual v1',assetForge:true,shaderVfxForge:true,discoveryEngine:true,trendRadar:true,themeRadar:true,pokiGameEvents:true,visualMinimum:78,visualCommercialTarget:85,aiPlaytester:true,retentionReplay:true,adReadiness:true,performanceGate:true,pokiReadiness:true,desktopViewport:'1440x900',phoneViewport:'390x844'},studioLoop:{version:STUDIO_LOOP_VERSION,defaultNewGames:true,defaultExistingEdits:false,...DEFAULT_STUDIO_LOOP},testFunnel:{version:TEST_FUNNEL_VERSION,realPlayerAuthority:true},snapshots:{automatic:true,maxPerGame:12,undo:true},gameTrash:{safeDelete:true,recoverable:true},activeGames:[...activeByGame.keys()],recentJobs:jobs.length});
  }
